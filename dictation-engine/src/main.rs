...
impl VoskEngine {
    fn new(model_path: &str) -> Result<Self> {
        ...
        // Warm up recognizer with silence to avoid missing first words
        let silence = vec![0i16; SAMPLE_RATE as usize / 10]; // 100ms of silence
    ...
    fn process_audio(&self, samples: &[i16]) -> Result<()> {
        // Store audio for correction pass later
        ...
        // When Vosk finalizes a chunk, accumulate it (with deduplication)
        if state == vosk::DecodingState::Finalized {
            ...
            if let Some(finalized) = result.single() {
                ...
                if !text.is_empty() {
                    ...
                    // Remove any duplicate overlap between accumulated and new chunk
    ...
    fn run_correction_pass(&self, accurate_model: &Model) -> Result<String> {
        ...
        // Create recognizer with preloaded model
        ...
        // Process all stored audio with accurate model
        let audio_buffer = self.audio_buffer.lock().unwrap();
        
        // Process in chunks
        const CHUNK_SIZE: usize = 8000; // ~0.5s chunks
    ...
    fn get_final_result(&self) -> Result<String> {
        ...
        // Get any remaining finalized text
...
async fn main() -> Result<()> {
    ...
    // Start audio capture IMMEDIATELY to buffer samples
    let capture = AudioCapture::new(audio_tx)?;
    capture.start()?;
    info!("Audio capture started - buffering...");
    
    // Load model while audio is being buffered
    ...
    // Preload accurate model in background while recording
    info!("Preloading accurate model in background...");
    let accurate_model_handle = tokio::spawn(async {
        Model::new(ACCURATE_MODEL_PATH)
    });
    ...
    // Collect buffered audio during model load
    ...
    let preview_task = tokio::spawn(async move {
        ...
        loop {
            ...
            match engine_clone.get_current_full_text() {
                Ok(text_curr) => {
                    let mut server = control_clone_for_preview.lock().await;
                    let _ = server.broadcast(&ControlMessage::TranscriptionUpdate {
                        text: text_curr,
                        is_final: false,
                    }).await;
                }
    ...
    let mut server = control_server_shared.lock().await;
    server.broadcast(&ControlMessage::TranscriptionUpdate {
