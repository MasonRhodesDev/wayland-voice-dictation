...
impl OverlayLayout {
    pub fn new(width: f32, initial_text_height: f32) -> Result<Self, taffy::TaffyError> {
        ...
        let text_node = taffy.new_leaf(Style {
            size: Size {
                width: length(width),
                height: length(initial_text_height),
            },
            ..Default::default()
        })?;
    ...
    pub fn update_text_height(&mut self, height: f32) -> Result<(), taffy::TaffyError> {
        self.taffy.set_style(
            ...
            Style {
                size: Size {
                    width: length(self.taffy.style(self.text_node)?.size.width.resolve_or_zero(None)),
    ...
    pub fn compute(&mut self, available_width: f32, available_height: f32) -> Result<(), taffy::TaffyError> {
