...
use dictation_gui::animations;
use dictation_gui::control_ipc::{self, ControlMessage};
use dictation_gui::fft::{self, SpectrumAnalyzer};
use dictation_gui::ipc;
use dictation_gui::wayland;
use dictation_gui::GuiState;
use dictation_gui::renderer_v2::ModernRenderer;

const SOCKET_PATH: &str = "/tmp/voice-dictation.sock";
const CONTROL_SOCKET_PATH: &str = "/tmp/voice-dictation-control.sock";
const WIDTH: u32 = 400;
const LISTENING_HEIGHT: u32 = 120;  // Height for listening mode with text
const SPINNER_HEIGHT: u32 = 100;    // Height for spinner/processing mode
...
fn run_wayland_window(
    ...
    let mut current_height = LISTENING_HEIGHT;
    ...
    let mut renderer = ModernRenderer::new(current_width, current_height)?;
    ...
    loop {
        ...
        if let Some(context) = &app_state.context {
            ...
            // Track state changes and resize window accordingly
            if current_state != previous_state {
                ...
                let new_height = match current_state {
                    GuiState::Listening => LISTENING_HEIGHT,
                    GuiState::Processing => SPINNER_HEIGHT,
                    GuiState::Closing => SPINNER_HEIGHT,
                };

                if new_height != current_height {
                    current_height = new_height;
                    renderer = ModernRenderer::new(current_width, current_height)?;
